...
class SlashedValidators:
    ...
    def process(
        ...
        for index in not_our_new_exited_slashed_indexes:
            print(
                f"🔪     validator {total_exited_slashed_index_to_validator[index].pubkey} is slashed"
            )

        for index in our_new_exited_slashed_indexes:
            message = f"🔕 Our validator {our_exited_slashed_index_to_validator[index].pubkey} is slashed"
            print(message)
